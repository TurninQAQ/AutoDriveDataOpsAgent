...
LATEST_VERSION="v1.1.0  2027-07-15:(new:None_base:None)"
...
def build_trigger_conf(ds):
    ...
    if "timeout_min" not in ds:
        raise RuntimeError(f"Dataset [{ds_name}] missing required config: timeout_min")
    timeout_min = positive_int(ds["timeout_min"], f"datasets.{ds_name}.timeout_min")
    conf = {
...
with scheduler_all:
    for ds in datasets_cfg:
        ...
        TriggerDagRunOperator(
            ...
            trigger_dag_id="batch_pipeline_universal",
...
for ds in datasets_cfg:
    ...
    # start_date=pendulum.datetime(2026, 6, 17, tz="UTC")
    scheduler_dag = DAG(
        dag_id=f"scheduler_{ds_name}",
        default_args=default_args,
        start_date=start_date,
        # schedule="@once",
        schedule=None,
