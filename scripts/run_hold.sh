...
RESULT_FILE="${DATA_DIR}/results_hold.json"
SLEEP_SEC="${HOLD_SLEEP_SEC:-20}"
SLEEP_FILE="${DATA_DIR}/hold_sleep_sec.txt"
PYTHON_BIN="${AIRFLOW_PYTHON:-python3}"

if [ -f "$SLEEP_FILE" ]; then
    RAW_SLEEP="$(tr -cd '0-9' < "$SLEEP_FILE" | head -c 5)"
    if [ -n "$RAW_SLEEP" ]; then
        SLEEP_SEC="$RAW_SLEEP"
    fi
fi

echo "[INFO] Stage HOLD | dataset=${DATASET_NAME} | data_dir=${DATA_DIR} | sleep_sec=${SLEEP_SEC}"
sleep "$SLEEP_SEC"
