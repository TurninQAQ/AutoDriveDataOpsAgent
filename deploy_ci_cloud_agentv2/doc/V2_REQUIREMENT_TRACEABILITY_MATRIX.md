...
|---|---|---|
| Real Qwen chat/Agent Provider request | `UNVERIFIED_EXTERNAL` | The production `qwen-plus` chat request reached DashScope but returned `AllocationQuota.FreeTierOnly`; no parser or Runtime safety bypass was introduced |
...
| Docker hosted build/runtime | `PASS` | Hosted run #21 (`32656130224`) built and ran the non-root image, health/readiness, SQLite, and same-volume smoke |
| Docker local build/run | `BLOCKED_EXTERNAL` | Local Docker Hub `python:3.12-slim` pull timed out |
| Hosted CI execution | `PASS` | Hosted run #21 (`32656130224`) passed Python 3.11/3.12, real-LangGraph, compile, wheel/import, container, and static jobs |
