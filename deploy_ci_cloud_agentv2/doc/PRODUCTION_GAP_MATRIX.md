...
| Provider | Deterministic and scripted offline providers | Structured HTTP/Qwen adapter, timeout, retry, rate-limit handling, auth isolation, telemetry | Strict json_schema request with qwen3.7-plus-2026-05-26, valid first decision, fresh single-READ CompletionGate pass, READ_TOOL_BATCH with three normalized READs, and bounded five-run sample (1 completed, 4 external provider-unavailable); local fake HTTP provider tests pass |
